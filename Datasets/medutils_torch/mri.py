...
def mriAdjointOp(kspace, smaps, mask, fft_axes=(-2, -1), coil_axis=-3):
    ...
    :param coil_axis: defines the axis of the coils
                        (and extended coil sensitivity maps if softSENSE is used)
...
def removePEOversampling(src, axes=(-2, -1), dicom_offset=DICOM_OFFSET):
    ...
    if PE_OS_crop == 0:
        ...
        return np.take(src, np.arange(int(PE_OS_crop) + dicom_offset, nPE - int(PE_OS_crop) + dicom_offset),
                       axis=axes[1])
...
def postprocess(src, axes=(-2, -1), dicom_offset=DICOM_OFFSET):
    ...
    return removePEOversampling(removeFEOversampling(src, axes, dicom_offset=dicom_offset), axes,
                                dicom_offset=dicom_offset)
